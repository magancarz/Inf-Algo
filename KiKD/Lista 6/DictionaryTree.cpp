...
DictionaryTree::DictionaryTree(std::vector<int> &values, int size) {
    this->root = new Node{"", 127, nullptr, nullptr};
    this->height = size;
...
void DictionaryTree::createNode(const std::vector<int>& values, Node* node) {
    ...
    node->left = new Node{node->code + "0", node->value, nullptr, nullptr};
    node->right = new Node{node->code + "1", node->value, nullptr, nullptr};
