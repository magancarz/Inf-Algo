...
int main(int argc, char* argv[])
{
	if (std::string(argv[3]) == "-ss")
	{
		...
		auto dijkstra_result = dijkstra_benchmark.dialDijkstraSourcesBenchmark();

		auto end = std::chrono::steady_clock::now();
	#ifdef DEBUG
		std::cout << "(Dial) time elapsed: "
	...
	else if (std::string(argv[3]) == "-p2p")
	{
		...
		auto dijkstra_result = dijkstra_benchmark.dialDijkstraPathsBenchmark();
